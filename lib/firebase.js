...
export const getTasks = async (userProfile, teamFilter = null) => {
    try {
        ...
        console.log(`Fetching tasks for ${userProfile.role} (${userProfile.empId}) from teams:`, accessibleTeams);
            ...
            try {
                ...
                for (const dateDoc of datesSnapshot.docs) {
                    ...
                    for (const empDoc of employeesSnapshot.docs) {
                        ...
                        console.log(`Found ${empTasks.length} tasks for employee ${empId} on ${date}`);
                    }
                }
            } catch (teamError) {
        ...
        console.log(`Total tasks fetched for ${userProfile.role}: ${allTasks.length}`);
...
export const getWeeklyTasks = async (userProfile, startDate, endDate) => {
    try {
        if (!userProfile) {
            throw new Error('User profile is required');
        }
