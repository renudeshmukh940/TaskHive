...
export const getAdminDashboardStats = async () => {
    try {
        ...
        const today = format(new Date(), 'yyyy-MM-dd');
        const last7Days = format(addDays(new Date(), -7), 'yyyy-MM-dd');
        ...
        for (const teamDoc of teamsSnapshot.docs) {
            ...
            // Get team employees
            const employeesSnapshot = await getDocs(
                collection(db, 'teams', teamName, 'employees')
            );
            ...
            // Get today's tasks for this team
            try {
                const todayTasksSnapshot = await getDocs(
                    collection(db, 'teams', teamName, 'dates', today, 'employees')
                );

                let teamTasksToday = 0;
                let teamTotalTime = 0;

                for (const empDoc of todayTasksSnapshot.docs) {
                    const tasksSnapshot = await getDocs(
                        collection(db, 'teams', teamName, 'dates', today, 'employees', empDoc.id, 'tasks')
                    );

                    teamTasksToday += tasksSnapshot.size;
                    stats.totalTasks += tasksSnapshot.size;
                    teamStats.tasksToday += tasksSnapshot.size;

                    // Calculate time spent
                    tasksSnapshot.docs.forEach(taskDoc => {
                        const task = taskDoc.data();
                        const timeSpent = parseFloat(task.timeSpent || 0);
                        teamTotalTime += timeSpent;

                        // Track project time
                        if (task.projectName && task.projectId) {
                            const projectKey = `${task.projectName}-${task.projectId}`;
                            stats.timeByProject[projectKey] = (stats.timeByProject[projectKey] || 0) + timeSpent;
                        }
                        ...
                        switch (task.status?.toLowerCase()) {
                            case 'completed':
                                ...
                                teamStats.projects.completed++;
                                break;
                            case 'in progress':
                                stats.projectStatus.inProgress++;
                                teamStats.projects.inProgress++;
                                break;
                            case 'on hold':
                                stats.projectStatus.onHold++;
                                teamStats.projects.onHold++;
                                break;
                        }
                    });
                }

                teamStats.projects.totalTime = teamTotalTime;
                stats.recentActivity.push({
                    team: teamName,
                    date: today,
                    tasks: teamTasksToday,
                    hours: teamTotalTime.toFixed(1)
                });
            } catch (error) {
                console.warn(`No tasks found for team ${teamName} today`);
            }

            // Get projects for this team
            try {
                const projectsSnapshot = await getDocs(
                    collection(db, 'teams', teamName, 'projects')
                );
                teamStats.projectCount = projectsSnapshot.size;
                stats.totalProjects += projectsSnapshot.size;
            } catch (error) {
                teamStats.projectCount = 0;
            }
...
// NEW: Get detailed team insights
export const getTeamInsights = async (teamName) => {
    try {
        const insights = {
            teamName,
            employeeCount: 0,
        ...
        insights.employeeCount = employeesSnapshot.size;

        // Get all dates with tasks
        const datesSnapshot = await getDocs(
            collection(db, 'teams', teamName, 'dates')
        );

        for (const dateDoc of datesSnapshot.docs) {
            const date = dateDoc.id;
            insights.activityByDate[date] = { tasks: 0, hours: 0 };
            ...
            for (const empDoc of employeesSnapshot.docs) {
                ...
                tasksSnapshot.docs.forEach(taskDoc => {
                    ...
                    const timeSpent = parseFloat(task.timeSpent || 0);
                    empHours += timeSpent;
                    insights.totalHours += timeSpent;
                    insights.activityByDate[date].tasks += 1;
                    insights.activityByDate[date].hours += timeSpent;

                    // Project tracking
                    if (task.projectName && task.projectId) {
                        const projectKey = `${task.projectName}-${task.projectId}`;
                        insights.timeByProject[projectKey] = (insights.timeByProject[projectKey] || 0) + timeSpent;

                        switch (task.status?.toLowerCase()) {
                            case 'completed':
                                insights.projects.completed++;
                                break;
                            case 'in progress':
                                insights.projects.inProgress++;
                                break;
                            case 'on hold':
                                insights.projects.onHold++;
                                break;
                        }
                    }
                ...
                if (empTasks > 0) {
                    insights.topPerformers.push({
                        empId,
                        empName: empDoc.data().name || empId,
                        tasks: empTasks,
                        hours: empHours
                    });
                }
        ...
        // Sort top performers
        insights.topPerformers.sort((a, b) => b.hours - a.hours);
        insights.topPerformers = insights.topPerformers.slice(0, 5);

        // Sort activity by date
        const sortedDates = Object.keys(insights.activityByDate).sort((a, b) => new Date(b) - new Date(a));
        insights.activityByDate = sortedDates.reduce((acc, date) => {
            acc[date] = insights.activityByDate[date];
            return acc;
        }, {});

        insights.projects.total = insights.projects.completed + insights.projects.inProgress + insights.projects.onHold;
