...
const PREDEFINED_VALUES = {
    ...
    workType: ['Full-day', 'Half-day', 'Relaxation', 'Over Time']
};
...
export const getTasksForEmployee = async (teamName, empId, date) => {
    try {
                ...
                try {
                    ...
                    for (const empDoc of dateEmployeesSnapshot.docs) {
                        ...
                        tasksSnapshot.docs.forEach(taskDoc => {
                            ...
                            if (task.timeSpent) {
                                // Parse user input HH:MM to hours
                                ...
                                // अगर user ने timeSpent नहीं डाला, तभी default लो
                                if (task.workType === 'Full-day') hours = 9;
                                else if (task.workType === 'Half-day') hours = 4.5;
                                else if (task.workType === 'Relaxation') hours = 7;
                                else if (task.workType === 'Over Time') hours = 11;
                            }
...
export const getTeamInsights = async (teamName, startDate, endDate) => {
    try {
        ...
        // Get team employees from users collection (exclude team-leader, handle special techLeads team)
        let teamMembersQuery;
        if (teamName === 'techLeads') {
            // For techLeads team: query by role (teamName is null for tech-leads)
            teamMembersQuery = query(
        ...
        const teamMembersSnapshot = await getDocs(teamMembersQuery);
        insights.employeeCount = teamMembersSnapshot.size;
